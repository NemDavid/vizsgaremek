    ...
    const rawPosts = [
        {
            ID: 1,
            USER_ID: 1,
            content: "(1) test post content",
            title: "(1) test post title",
        },
        {
            ID: 2,
            USER_ID: 2,
            content: "(2) test post content",
            title: "(2) test post title",
        },
        {
            ID: 3,
            USER_ID: 2,
            content: "(3) test post content",
            title: "(3) test post title",
        },
    ];
    ...
    describe("GET", () => {
        describe("GET /api/reactions", () => {
            test("should get all reactions in db", async () => {
                const res = await request(app).get("/api/reactions").set("Cookie", [cookie]).expect(200);

                expect(res.body).toBeDefined();
                expect(res.body.length).toBe(4);
                expect(res.body).toEqual(
                    expect.arrayContaining(
                        rawPostsReactions.map((reaction) =>
    ...
    describe("DELETE", () => {
        describe("DELETE /api/reactions", () => {
            test("should delete reaction by ID", async () => {
                ...
                await request(app).delete(`/api/reactions/${itemId}`).set("Cookie", [cookie]).expect(200);
            ...
            test("should throw ValidationError when token is invalid", async () => {
                ...
                const res = await request(app).delete(`/api/reactions/${itemId}`).set("Cookie", [cookieinvalid]).expect(403);
                expect(res.body).toBeDefined();
                expect(res.body.message).toBeDefined();
                expect(res.body.message).toBe("Hiányzó vagy lejárt token.");
            });
            test("should throw UnauthorizedError when missing token", async () => {
                ...
                const res = await request(app).delete(`/api/reactions/${itemId}`).expect(401);
                expect(res.body).toBeDefined();
                expect(res.body.message).toBeDefined();
                expect(res.body.message).toBe("Hiányzó user token");
            });
