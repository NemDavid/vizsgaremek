    ...
    describe("GET", () => {
        describe("GET /api/comments", () => {
            test("should get all comments from db", async () => {
                const res = await request(app).get("/api/comments").set("Cookie", [cookie]).expect(200);
            ...
            test("should throw ValidationError when token is invalid", async () => {
                const res = await request(app).get("/api/comments").set("Cookie", [cookieinvalid]).expect(403);

                expect(res.body).toBeDefined();
                expect(res.body.message).toBeDefined();
            });
            test("should throw UnauthorizedError when missing token", async () => {
                const res = await request(app).get("/api/comments").expect(401);
                expect(res.body).toBeDefined();
                expect(res.body.message).toBeDefined();
            });
        });
