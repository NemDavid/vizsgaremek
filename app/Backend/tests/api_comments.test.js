        ...
        describe("GET", () => {
                ...
                test("should get all comments from db", async () => {
                    ...
                    expect(res.body).toEqual(
                        expect.arrayContaining(
                            rawComments.map(comment => expect.objectContaining({
            ...
            describe("GET /api/comments/:itemId", () => {
                test("should get one comment from db by id", async () => {
                    const itemId = 1;
                    const res = await request(app).get(`/api/comments/${itemId}`).expect(200);
            ...
            describe("GET /api/comments/postComments/:itemId", () => {
                test("should get one post comments by post id", async () => {
                    const itemId = 2;
                    const res = await request(app).get(`/api/comments/postComments/${itemId}`).expect(200);
                    ...
                    expect(res.body).toEqual(
                        expect.arrayContaining(
                            foundComments.map(comment => expect.objectContaining({
                                USER_ID: comment.USER_ID,
                                POST_ID: comment.POST_ID,
                                comment: comment.comment,
                            })))
                ...
                test("should throw error on invalid post id", async () => {
                    const itemId = 9999;
                    const res = await request(app).get(`/api/comments/postComments/${itemId}`).expect(400);
        ...
        describe("POST", () => {
            describe("POST /api/comments", () => {
                test("should create a new comment", async () => {
                    ...
                    const res = await request(app).post("/api/comments").send(commentData).set("Cookie", [cookie]).expect(201);
                ...
                test("should throw error on invalid token", async () => {
                    ...
                    const res = await request(app).post("/api/comments").send(commentData).set("Cookie", [cookie]).expect(400);
                ...
                test.each([
                    ...
                    const res = await request(app).post("/api/comments").send(payload).set("Cookie", [cookie]).expect(400);
