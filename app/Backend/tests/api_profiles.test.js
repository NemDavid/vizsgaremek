...
describe("/api/profiles", () => {
    ...
    beforeEach(async () => {
        ...
        token = authUtils.generateUserToken(rawSingleUsers);
    ...
    describe("GET", () => {
        ...
        describe("GET /api/profiles/pages/:paramPage", () => {
            test("should return the correct profiles", async () => {
                ...
                const res = await request(app)
                    .get(`/api/profiles/pages/${paramPage}`)
                    .set("Cookie", [cookie])
                    .set("Accept", "application/json");
    ...
    describe("POST", () => {
        test("should create a new profile", async () => {
            ...
            await request(app).post("/api/profiles").send(profile).set("Cookie", [cookie]).expect(201);
        ...
        test.each([
            ...
            "should throw error on missing name",
        ...
        test.each([
            [
                { USER_ID: 3, first_name: 1, last_name: "Kiss" },
                "Érvénytelen first_name",
            ],
            [
                { USER_ID: 3, first_name: "Anna", last_name: 2 },
                "Érvénytelen last_name",
            ],
            [
                { USER_ID: 3, first_name: "Anna", last_name: "Kiss", schools: 3 },
                "Érvénytelen schools mező",
            ],
            [
                {
                    USER_ID: 3,
                ...
                "Érvénytelen birth_date mező",
            ],
            [
                { USER_ID: 3, first_name: "Anna", last_name: "Kiss", birth_place: 4 },
                "Érvénytelen birth_place mező",
            ],
            [
                { USER_ID: 3, first_name: "Anna", last_name: "Kiss", bio: 5 },
                "Érvénytelen bio",
            ],
        ])(
    ...
    describe("DELETE", () => {
        test("should delete profile from db", async () => {
            ...
            await request(app).delete(`/api/profiles/${inputID}`).set("Cookie", [cookie]).expect(200);
    ...
    describe("UPDATE", () => {
        ...
        test.each([
            [
                { first_name: undefined, last_name: "update_Kovács" },
                "Hiányzó first_name",
            ],
            [
                { first_name: "update_Gergő", last_name: undefined },
                "Hiányzó last_name",
            ],
        ])(
            "should throw error on missing name",
        ...
        test.each([
            [
                { first_name: 1, last_name: "update_Kovács" },
                "Érvénytelen first_name",
            ],
            ...
            [
                { first_name: "update_Gergő", last_name: "update_Kovács", bio: 5 },
                "Érvénytelen bio",
            ],
        ])(
            "should throw error on invalid attributes",
    ...
    describe("XP function tests", () => {
        test("should add xp to profile", async () => {
            ...
            const { profile: updatedProfile } =
                await user_profileService.getUser_Profile(inputID);
        ...
        test.each([
            ...
            "should throw ValidationError error in model on invalid xp",
            async (XPAmount, inputID) => {
                try {
                    const { profile } =
                        await user_profileService.getUser_Profile(inputID);
