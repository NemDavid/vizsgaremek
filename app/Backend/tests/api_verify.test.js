...
describe("user_settings_Controller", () => {
    ...
    const codes = [
        { ID: 1, email: "admin@example.com", verify_code_hash: authUtils.hashCode("" + authUtils.generateVerifyCode()) },
    ];
        ...
        describe("GET", () => {
            describe("GET /api/verify", () => {
                test("should get all codes in db", async () => {
                    ...
                    expect(res.body).toEqual(expect.arrayContaining(
                        codes.map(code => expect.objectContaining({
                            email: code.email,
            ...
            describe("GET /api/verify/:itemId", () => {
                test("should get one code by itemId", async () => {
                    ...
                    const res = await request(app).get(`/api/verify/${itemId}`).expect(200);
            ...
            describe("GET /api/verify/Email", () => {
                test("should get one code to its email", async () => {
                    ...
                    const res = await request(app).get("/api/verify/Email").send({ email }).set("Cookie", [cookie]).expect(200);
                ...
                test("should throw error on missing email", async () => {
                    ...
                    const res = await request(app).get("/api/verify/Email").send({ email }).set("Cookie", [cookie]).expect(400);
        ...
        describe("POST", () => {
            describe("POST /api/verify", () => {
                test("should create code", async () => {
                    ...
                    const res = await request(app).post("/api/verify").send({ email }).expect(201);
                ...
                test("should throw error on missing email", async () => {
                    const email = undefined;
                    const res = await request(app).post("/api/verify").send({ email }).expect(400);
        ...
        describe("DELETE", () => {
            describe("DELETE /api/verify", () => {
                ...
                test("should thorw error on email that hasn't code", async () => {
                    const email = rawUsers[1].email;
                    const res = await request(app).delete("/api/verify").send({ email }).expect(400);
            ...
            describe("DELETE /api/verify/:itemId", () => {
                ...
                test("should throw error on invalid itemId", async () => {
                    const itemId = 9999;
                    const res = await request(app).delete(`/api/verify/${itemId}`).expect(400);
        ...
        describe("UPDATE", () => {
            describe("UPDATE /api/verify/email/:email", () => {
                test("should update code by itemId", async () => {
                    ...
                    await request(app).patch(`/api/verify/email/${email}`).send({ verify_code }).expect(200);
                    ...
                    expect(bcrypt.compareSync(verify_code, foundCode.verify_code_hash)).toBeTruthy();
                })
                ...
                test("should throw error on email that hasn't code", async () => {
                    ...
                    const res = await request(app).patch(`/api/verify/email/${email}`).send({ verify_code }).expect(400);
                ...
                test("should generate code on missing code input", async () => {
                    ...
                    const res = await request(app).patch(`/api/verify/email/${email}`).send({ verify_code }).expect(200);
